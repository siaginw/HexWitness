...
5. Use callers, callees, and xrefs only after the entity is resolved.
6. Check `hexwitness_evidence` and `hexwitness_contradictions` before stating a conclusion.
7. Label outputs as proven, strongly inferred, provisional, contradicted, or unknown.
8. When evidence is missing, request the smallest bounded export or runtime observation that can close the gap.
